defmodule CouchGears.Database do
  ...
  def find_with_rev(db, doc_id, rev) do
    { _, document } = do_find_with_rev(doc_id, rev, db)
    document
  end
  ...
  defp do_find_with_rev(ddoc, rev, database(raw_db: raw_db)) do
    case :couch_db.open_doc_revs(raw_db, ddoc, make_rev(rev), []) do
      {:ok, [{:ok, doc}]} ->
        {body} = :couch_doc.to_json_obj(doc, [])
